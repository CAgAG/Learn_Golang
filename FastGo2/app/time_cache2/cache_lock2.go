...
func (lru *LRUCache) Get(key string) (interface{}, bool) {
	element, ok := lru.cache.Load(key)
	if !ok {
		return nil, false
	}

	entry := element.(*list.Element).Value.(*CacheEntry)
	if time.Now().After(entry.lastUsed.Add(entry.expiration)) {
		// 条目已过期，从缓存中移除
		lru.removeElement(element.(*list.Element))
		return nil, false
	}
	entry.lastUsed = time.Now()
	lru.data.MoveToFront(element.(*list.Element))
	return entry.value, true
}

// Set 设置一个缓存值，如果缓存已满，则移除最久未使用的条目
func (lru *LRUCache) Set(key string, value interface{}, expiration time.Duration) {
	if element, ok := lru.cache.Load(key); ok {
		lru.data.MoveToFront(element.(*list.Element))
		entry := element.(*list.Element).Value.(*CacheEntry)
		entry.value = value
